...
public class RoleController {
    ...
    @Operation(summary = "Add role to user", security = @SecurityRequirement(name = JWT_AUTH))
    @PostMapping
    public Response<UserWithRoles> addRoleToUser(Long userId, String roleName) {
        return roleService.addRoleToUser(userId, roleName);
    }

    @Operation(summary = "Remove role from user", security = @SecurityRequirement(name = JWT_AUTH))
    @DeleteMapping
